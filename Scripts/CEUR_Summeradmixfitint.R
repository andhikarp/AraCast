...
summer126=matrix(NA,nrow=3015,ncol=1)
dtbsummer126=matrix(NA,nrow=3015,ncol=1)
zeron26=matrix(NA,nrow=1,ncol=1)	
# nacount45=matrix(NA,nrow=1,ncol=1)
# summer145=matrix(NA,nrow=3015,ncol=1)
# dtbsummer145=matrix(NA,nrow=3015,ncol=1)
# zeron45=matrix(NA,nrow=1,ncol=1)
# nacount60=matrix(NA,nrow=1,ncol=1)
# summer160=matrix(NA,nrow=3015,ncol=1)
# dtbsummer160=matrix(NA,nrow=3015,ncol=1)
# zeron60=matrix(NA,nrow=1,ncol=1)
nacount85=matrix(NA,nrow=1,ncol=1)
summer185=matrix(NA,nrow=3015,ncol=1)
dtbsummer185=matrix(NA,nrow=3015,ncol=1)
zeron85=matrix(NA,nrow=1,ncol=1)
for (i in c(26,85)){
	startdate= 145 #sowing date
	...
	dtb_preds[dtb_preds<16]=NA #outside empirical range
	dtb_preds[dtb_preds>246]=NA 

	#calculating seed number
	...
	assign(paste0('summer1',i),cbind(eval(as.name(paste0('summer1',i))),sn_preds))
	assign(paste0('dtbsummer1',i),cbind(eval(as.name(paste0('dtbsummer1',i))),dtb_preds))
...
tmins=tmin[[(145+(365*6)):((145+365*6)+202)]]
tmins=mask(tmins,clusters[[2]])
tmins=mask(tmins,europe)
tmaxs=tmax[[(145+(365*6)):((145+365*6)+202)]]
...
dtb_preds[dtb_preds>246]=NA 
summer12006=(vvbs%*%snbetas) + (values %*% snw_ridge)+ sn_intercept
summer12006=matrix(summer12006,nrow=3015,ncol=1)
dtbsummer12006=matrix(dtb_preds,nrow=3015,ncol=1)
nacount=sum(is.na(sn_preds))
zeron=sum(na.omit(sn_preds)==0)

summer126=summer126[,-1]
summer145=summer145[,-1]
summer160=summer160[,-1]
summer185=summer185[,-1]

dtbsummer126=dtbsummer126[,-1]
dtbsummer145=dtbsummer145[,-1]
dtbsummer160=dtbsummer160[,-1]
dtbsummer185=dtbsummer185[,-1]

save(dtbsummer12006,dtbsummer126,dtbsummer185,summer12006,summer126,summer185,file="~/Clim_GWAS/Clim_GWAS_2/CEUR_summerleapadmixfitint.RData")

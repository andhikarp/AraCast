#A model without environmental predictors and K = 0.1wg, to show what happens when you don't include environmental data
import sys
...
for_plotting.to_csv('DTBASE_pvo_nullEnv.csv',header=True)
...
np.save("Weights_DTBASE_nullEnv.npy",weights)
np.save("Weightsridge_DTBASE_nullEnv.npy",wride)
...
resultspd.to_csv("Results_DTBASE_nullEnv.csv")

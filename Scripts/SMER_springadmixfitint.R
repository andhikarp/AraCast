...
SMERspring26=matrix(NA,nrow=3015,ncol=1)
dtbSMERspring26=matrix(NA,nrow=3015,ncol=1)
zeron26=matrix(NA,nrow=1,ncol=1)	
# nacount45=matrix(NA,nrow=1,ncol=1)
# SMERspring45=matrix(NA,nrow=3015,ncol=1)
# dtbSMERspring45=matrix(NA,nrow=3015,ncol=1)
# zeron45=matrix(NA,nrow=1,ncol=1)
# nacount60=matrix(NA,nrow=1,ncol=1)
# SMERspring60=matrix(NA,nrow=3015,ncol=1)
# dtbSMERspring60=matrix(NA,nrow=3015,ncol=1)
zeron60=matrix(NA,nrow=1,ncol=1)
nacount85=matrix(NA,nrow=1,ncol=1)
SMERspring85=matrix(NA,nrow=3015,ncol=1)
dtbSMERspring85=matrix(NA,nrow=3015,ncol=1)
...
for (i in c(26,85)){
	...
	assign(paste0('SMERspring',i),cbind(eval(as.name(paste0('SMERspring',i))),sn_preds))
	assign(paste0('dtbSMERspring',i),cbind(eval(as.name(paste0('dtbSMERspring',i))),dtb_preds))
...
#dtb_preds[dtb_preds<16]=NA 
#dtb_preds[dtb_preds>246]=NA 
SMERspring2006=(vvbs%*%snbetas) + (values %*% snw_ridge) + sn_intercept
SMERspring2006[SMERspring2006<0]=0
SMERspring2006=matrix(SMERspring2006,nrow=3015,ncol=1)
dtbSMERspring2006=matrix(dtb_preds,nrow=3015,ncol=1)
nacount=sum(is.na(sn_preds))
zeron=sum(na.omit(sn_preds)==0)


SMERspring26=SMERspring26[,-1]
SMERspring45=SMERspring45[,-1]
SMERspring60=SMERspring60[,-1]
SMERspring85=SMERspring85[,-1]
dtbSMERspring26=dtbSMERspring26[,-1]
# dtbSMERspring45=dtbSMERspring45[,-1]
# dtbSMERspring60=dtbSMERspring60[,-1]
dtbSMERspring85=dtbSMERspring85[,-1]

save(dtbSMERspring2006,dtbSMERspring26,dtbSMERspring85,SMERspring2006,SMERspring26,SMERspring85,file="~/Clim_GWAS/Clim_GWAS_2/SMER_springleapadmixfitint.RData")

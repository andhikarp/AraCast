#A model where K = all zeros, to show what happens if you just use environmental data
import sys
...
for_plotting.to_csv('DTBASE_pvo_nullKone.csv',header=True)
...
np.save("Weights_DTBASE_nullKone.npy",weights)
np.save("Weightsridge_DTBASE_nullKone.npy",wride)
...
resultspd.to_csv("Results_DTBASE_nullKone.csv")

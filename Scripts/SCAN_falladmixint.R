...
SCANfall26=matrix(NA,nrow=3015,ncol=1)
dtbSCANfall26=matrix(NA,nrow=3015,ncol=1)
zeron26=matrix(NA,nrow=1,ncol=1)	
# nacount45=matrix(NA,nrow=1,ncol=1)
# SCANfall45=matrix(NA,nrow=3015,ncol=1)
# dtbSCANfall45=matrix(NA,nrow=3015,ncol=1)
# zeron45=matrix(NA,nrow=1,ncol=1)
# nacount60=matrix(NA,nrow=1,ncol=1)
# SCANfall60=matrix(NA,nrow=3015,ncol=1)
# dtbSCANfall60=matrix(NA,nrow=3015,ncol=1)
# zeron60=matrix(NA,nrow=1,ncol=1)
nacount85=matrix(NA,nrow=1,ncol=1)
SCANfall85=matrix(NA,nrow=3015,ncol=1)
dtbSCANfall85=matrix(NA,nrow=3015,ncol=1)
...
for (i in c(26,85)){
	...
	assign(paste0('SCANfall',i),cbind(eval(as.name(paste0('SCANfall',i))),sn_preds))
	assign(paste0('dtbSCANfall',i),cbind(eval(as.name(paste0('dtbSCANfall',i))),dtb_preds))
...
# dtb_preds[dtb_preds<16]=NA 
# dtb_preds[dtb_preds>246]=NA 
SCANfall2006=(vvbs%*%snbetas) + (values %*% snw_ridge) + sn_intercept
SCANfall2006[SCANfall2006<0]=0
SCANfall2006=matrix(SCANfall2006,nrow=3015,ncol=1)
dtbSCANfall2006=matrix(dtb_preds,nrow=3015,ncol=1)
nacount=sum(is.na(sn_preds))
zeron=sum(na.omit(sn_preds)==0)

SCANfall26=SCANfall26[,-1]
SCANfall45=SCANfall45[,-1]
SCANfall60=SCANfall60[,-1]
SCANfall85=SCANfall85[,-1]
dtbSCANfall26=dtbSCANfall26[,-1]
dtbSCANfall45=dtbSCANfall45[,-1]
dtbSCANfall60=dtbSCANfall60[,-1]
dtbSCANfall85=dtbSCANfall85[,-1]

save(dtbSCANfall2006,dtbSCANfall26,dtbSCANfall85,SCANfall2006,SCANfall26,SCANfall85,file="~/Clim_GWAS/Clim_GWAS_2/SCAN_fallleapadmixfitint.RData")
